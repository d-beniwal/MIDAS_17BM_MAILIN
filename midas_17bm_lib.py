"""Shared calibration/integration logic for `midas_17bm_pipeline.py`.
...
from midas_integrate_v2.corrections import polarization_factor
...
def r_px_from_two_theta_deg(two_theta_deg, Lsd_um, pxY_um):
    """Flat-panel px radius for a given 2theta (deg), given the calibration's
    sample-to-detector distance and pixel pitch."""
    return Lsd_um / pxY_um * math.tan(math.radians(two_theta_deg))


def resolve_r_min_px(calibration_json_path, r_min_px, r_min_two_theta_deg=None):
    """Honor R_MIN_2THETA_DEG over R_MIN_PX when the former is set (not
    None) -- converts using the calibration's own Lsd/pxY."""
    if r_min_two_theta_deg is None:
        return r_min_px
    with open(calibration_json_path) as fh:
        c = json.load(fh)
    return r_px_from_two_theta_deg(r_min_two_theta_deg, c['Lsd_um'], c['pxY_um'])


def full_ring_r_max_px(calibration_json_path):
    """Radius (px) of the largest circle around the beam centre that stays
    fully on-detector -- i.e. the nearest-edge distance. Rings beyond this
    are clipped (not full 360 deg) and bias an azimuthally-averaged lineout."""
    with open(calibration_json_path) as fh:
        c = json.load(fh)
    BC_y, BC_z = c['BC_y_px'], c['BC_z_px']
    NY, NZ = c['NrPixelsY'], c['NrPixelsZ']
    return min(BC_y, (NY - 1) - BC_y, BC_z, (NZ - 1) - BC_z)
...
class IntegrationContext:
    ...
    (spec_from_calibration_json + the detector-mapping `geom` object), so
    build it once per calibration and reuse across every frame that shares
    it (see integrate_with_context / midas_17bm_pipeline.py)."""
    spec: object
    geom: object
    binning_cfg: dict
    method: str
...
def build_integration_context(calibration_json_path, *,
    ...
    pol_factor = None
    if polarization:
        R_px, eta_deg = eval_pixel_REta(spec)
        pol_factor = polarization_factor(
            R_px, eta_deg, Lsd=spec.Lsd, px=torch.as_tensor(spec.pxY, dtype=torch.float64),
            pol_fraction=torch.as_tensor(pol_fraction, dtype=torch.float64),
...
def integrate_with_context(image, context: IntegrationContext, *,
                            error_model='poisson',
                            pixel_weighted_averaging=True) -> IntegrationResult:
    """Integrate `image` using an already-built IntegrationContext -- the
    detector mapping (context.geom) is reused as-is, not rebuilt. `image`
    can be any frame on this detector -- a fresh sample exposure, not just
    the calibrant.
    """
    spec, geom, cfg, method = context.spec, context.geom, context.binning_cfg, context.method
...
def integrate_frame(image, calibration_json_path, *,
    ...
    IntegrationContext every call) -- see build_integration_context() /
    integrate_with_context() to reuse one context across multiple frames.
    """
    context = build_integration_context(
        calibration_json_path, method=method,
        r_bin_size=r_bin_size, eta_bin_size=eta_bin_size,
